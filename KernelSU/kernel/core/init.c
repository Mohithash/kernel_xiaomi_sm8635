...
int __init kernelsu_init(void)
{
    ...
    // ksu_late_loaded does not exist in this tree (this codebase has no
    // late-load/LKM path implemented); we are always built-in, so only the
    // non-late-loaded path below is reachable. The dead branch referencing
    // it has been removed rather than left to bitrot further.
